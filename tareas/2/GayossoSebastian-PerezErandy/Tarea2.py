##Gayosso Rosillo Sebastian Emiliano y Pérez Delgado Erandy Estefanya
...
def generar_procesos(procesos, n, p, tmin, tmax):
    ...
    while n > 0:
        if randint(0, p-1) == 0:
            proceso = [x, randint(tmin, tmax), chr(nombre_ascii), 0, 0, 0, ""]
...
def fcfs(tareas):
    ...
    while tareas_entrada_ite < len(tareas_entrada) or len(tareas_cola) != 0:
        if tareas_entrada_ite < len(tareas_entrada):
            if tareas_entrada[tareas_entrada_ite][0] == tar:
...
def round_robin_n(tareas, n):
    ...
    t = 0  # Tiempo actual
    esquema = ""
    tareas_cola = []  # Cola de procesos listos
    tareas_entrada_ite = 0  # Iterador para las tareas de entrada
    quantum = n
    encolar = None
    max_tiempo = 1000  # Límite de seguridad para evitar bucles infinitos

    while (tareas_entrada_ite < len(tareas_entrada) or len(tareas_cola) != 0 or encolar is not None):
        if t > max_tiempo:
            print("Error: Se alcanzó el tiempo máximo de ejecución.")
            break
        ...
        # Procesar el primer proceso en la cola si hay alguno
        if len(tareas_cola) > 0:
            esquema += tareas_cola[0][2]  # Agregar al esquema el nombre del proceso
            for i in tareas_cola:
                i[3] += 1  # Incrementar tiempo de ejecución (T)

            tareas_cola[0][1] -= 1  # Reducir el tiempo restante del primer proceso
            quantum -= 1

            # Agregar el proceso al diagrama
            tareas_cola[0][6] += tareas_cola[0][2]
            for i in range(1, len(tareas_cola)):
                tareas_cola[i][6] += '-'
            ...
            if tareas_cola[0][1] == 0:
                ...
                for i in range(len(tareas_entrada)):
                    if tareas_entrada[i][2] == nombre:
                        tareas_entrada[i][3] = tareas_cola[0][3]  # Asignar T
                        tareas_entrada[i][6] = tareas_cola[0][6]  # Asignar diagrama
                        break
                tareas_cola.pop(0)  # Eliminar proceso de la cola
                quantum = n  # Reiniciar quantum

            # Si se acabó el quantum pero no el tiempo del proceso
            elif quantum == 0:
                encolar = tareas_cola.pop(0)  # Sacar el proceso para reinsertarlo
                quantum = n

        else:
            esquema += "i"  # Tiempo de inactividad

        # Reinsertar proceso encolado, si lo hay
